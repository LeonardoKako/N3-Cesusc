...
exports.criar = async (req, res) => {
  try {
    const { nome, descricao, preco, quantidade_estoque, data_validade, categoria_id, fornecedor_id } = req.body;

    if (!nome || preco <= 0 || quantidade_estoque < 0 || !categoria_id || !fornecedor_id) {
...
exports.atualizar = async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const { nome, descricao, preco, quantidade_estoque, categoria_id, fornecedor_id, data_validade } = req.body;
...
exports.deletar = async (req, res) => {
  try {
    ...
    const result = await checkRequest.query('SELECT quantidade_estoque FROM Produto WHERE id = @id');
